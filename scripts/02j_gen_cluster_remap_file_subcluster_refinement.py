...
# subclusters, to be annotated for cluster refinement.
...
# 01b.iii. Output (post-subclustering) epression data directory
input_data_dir = qc_root_dir + f'/' + cfg.get(f'output_data_dir__{pn}')
# 01b.iv. Output expression data file name
...
# 01b.i. Cluster label to remap from (post-subclustering but
# pre-recombination)
clust_label = cfg.get(f'output_cluster_label__{pn}')
# 01b.ii. Name of cell typing map file
...
cluster_mapping_df = pd.DataFrame(
        data={
            ...
            f'new_{clust_label}':None # to be filled in
            }
        )
